...
public class AppReceiverSettingsActivity extends PreferenceActivity {
    ...
    public static class Settings extends PreferenceFragment {
        ...
        /* renamed from: com.pyler.betterbatterysaver.activities.AppReceiverSettingsActivity.Settings.1 */
        class C00011 implements OnPreferenceChangeListener {
            final /* synthetic */ boolean val$isReceiverEnabled;
            final /* synthetic */ String val$serviceComponentName;

            C00011(boolean z, String str) {
                this.val$isReceiverEnabled = z;
                this.val$serviceComponentName = str;
            }

            public boolean onPreferenceChange(Preference preference, Object newValue) {
                DeviceController device = new DeviceController(AppReceiverSettingsActivity.mContext);
                if (((Boolean) newValue).booleanValue()) {
                    if (!this.val$isReceiverEnabled) {
                        device.setServiceMode(this.val$serviceComponentName, true);
                    }
                } else if (this.val$isReceiverEnabled) {
        ...
        public void onCreate(Bundle savedInstanceState) {
            ...
            AppReceiverSettingsActivity.mPrefs = PreferenceManager.getDefaultSharedPreferences(AppReceiverSettingsActivity.mContext);
            PreferenceScreen appServices = (PreferenceScreen) findPreference("app_services");
            PackageManager pm = AppReceiverSettingsActivity.mContext.getPackageManager();
            PackageInfo pi = null;
            try {
                pi = pm.getPackageInfo(AppReceiverSettingsActivity.packageName, 2);
            ...
            if (receiverList != null) {
                for (ActivityInfo receiver : receiverList) {
                    CheckBoxPreference serviceSetting = new CheckBoxPreference(AppReceiverSettingsActivity.mContext);
                    String longName = receiver.name;
                    String shortName = longName.substring(longName.lastIndexOf(".") + 1, longName.length());
                    ComponentName cn = new ComponentName(AppReceiverSettingsActivity.packageName, longName);
                    boolean isReceiverEnabled = pm.getComponentEnabledSetting(cn) != 2;
                    ...
                    serviceSetting.setOnPreferenceChangeListener(new C00011(isReceiverEnabled, serviceComponentName));
        ...
        public void onPause() {
            super.onPause();
            AppReceiverSettingsActivity.mUtils.setPrefsFileWorldReadable();
        }
    }
